...
function customerInvoice(raw: any): AccountsCustomerInvoice {
  return {
    ...
    baseAmount: numberValue(raw?.baseAmount, numberValue(raw?.amount)),
    baseCurrency: stringValue(raw?.baseCurrency, "GBP"),
...
function supplierPayment(raw: any): AccountsSupplierPayment {
  return {
    ...
    amount: numberValue(raw?.amount),
    baseAmount: numberValue(raw?.baseAmount, numberValue(raw?.amount)),
    baseCurrency: stringValue(raw?.baseCurrency, stringValue(raw?.currency, "GBP")),
    exchangeRate: numberValue(raw?.exchangeRate, 1),
    currency: stringValue(raw?.currency, "GBP"),
